...
class StaticAnalyzer:
        ...
        for rule in self.ir.rules:
            ...
            for cond in rule.conditions:
                if cond.predicate.name not in defined_names:
                    self.warnings.append(
                        f"Undefined concept '{cond.predicate.name}' in condition of rule {rule.id}"
                    )

            # Check conclusion
            if rule.conclusion.predicate.name not in defined_names:
                self.warnings.append(
                    f"Undefined concept '{rule.conclusion.predicate.name}' in conclusion of rule {rule.id}"
                )
            ...
            for exc in rule.exceptions:
                if exc.condition.predicate.name not in defined_names:
                    self.warnings.append(
                        f"Undefined concept '{exc.condition.predicate.name}' in exception of rule {rule.id}"
                    )
    ...
    def _check_cycles(self):
        ...
        try:
            ...
            self.warnings.append(
                f"Dependency cycle detected among rules: {' -> '.join(cycle_nodes)}"
            )
